...
def predicted_value_plot(model, df, column, classification=False, discrete_col=False, class_col=None, class_labels=None, bin_labels=None, freq=True, scatter=True, ci=(5, 95), scatter_num=None, num_linspace=100, response_label=None, cmap=None, figsize=(10, 7)):
    ...
    def _discrete_value_plot(scatter_num, bin_labels):
        ...
        # If a dictionary wasn't passed in for the bin_labels, set each to that label's value
        ...
        for label in labels:
            ...
            if classification:
                ...
                preds.append(np.array([boot_sample.mean(axis=0) for boot_sample in (resample(pred) for _ in xrange(1000))]).flatten())
        ...
        # If scatter is set to True, set up variable that will dictate the number of points in a particular scatter plot which will be used below
        if scatter:
        ...
        if classification and not class_col:
            ...
            for class_num in xrange(num_classes):
                bp_positions = range(class_num+1, num_boxplots+1, num_classes+1)

                # plt.boxplot only likes getting a list of arrays
                data = [preds[idx, :, class_num] for idx in range(preds.shape[0])]
                ...
                for box in bp['boxes']:
                    ...
                    boxPolygon = Polygon(boxCoords, facecolor=cmap[class_num], alpha=0.7)
                    ax1.add_patch(boxPolygon)

                # If scatter is set to true, plot the scatter for each of the bins for this particular class
                if scatter:
                    for idx, num in enumerate(num_per_label):
                        y_data = np.random.choice(preds[idx, :, class_num].flatten(), size=num)
                        x_data = [bp['whiskers'][idx*2].get_xdata()[0]] * num
                        jittered_x = _rand_jitter(x_data, bp['boxes'][idx])
                        ax1.scatter(jittered_x, y_data, c=cmap[class_num], alpha=0.6)
            ...
            # Set up the legend
            patches = [Patch(color=cmap[idx], alpha=0.7, label=class_labels.get(idx, idx)) for idx in range(num_classes)]
            ...
            if classification:
                # Set up the legend
                patch = Patch(color=cmap[0], alpha=0.7, label=class_labels.get(class_col, class_col))
    ...
    def _contin_value_plot():
        ...
        # If preds.shape == 2 then we only need one line and confidence interval
        if len(preds.shape) == 2:
            ...
            for col_idx in xrange(preds.shape[2]):
                ...
                ax1.plot(x_i, pred_means[:, col_idx], c=cmap[col_idx], linewidth=2, label=class_labels.get(col_idx, col_idx))
                ax1.legend(loc='best')
...
if __name__=='__main__':
    ...
    predicted_value_plot(rf, df, 'carat', response_label="Predicted Price in U.S. Dollars")
    ...
    predicted_value_plot(rf, df, 'cut', response_label="Predicted Price in U.S. Dollars", discrete_col=True, bin_labels=bin_labels)
    ...
    # Run on a continous column
    predicted_value_plot(rf, df, 'price', classification=True, class_labels=class_labels)
    plt.savefig('./imgs/classification_continuous_predicted_prob_plot.png', dpi=300)

    # Run on a discrete column
    predicted_value_plot(rf, df, 'color_E', classification=True, discrete_col=True, class_labels=class_labels)
    plt.savefig('./imgs/classification_discrete_predicted_prob_plot1.png', dpi=300)

    # Run on a discrete column looking at a particular class
    predicted_value_plot(rf, df, 'color_E', classification=True, discrete_col=True, class_labels=class_labels, class_col=1)
    plt.savefig('./imgs/classification_discrete_predicted_prob_plot2.png', dpi=300)
